...
engine = pyttsx3.init()
player_money = 120000
print(Back.LIGHTMAGENTA_EX + "\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t😊😊WELCOME TO THIS INTERESTING BIDDING GAME😊😊")
print(Style.RESET_ALL)
print(Fore.LIGHTRED_EX + "Rules💫")
print(Style.RESET_ALL)

def rules():
    print("There are 6 topics in this game")
    print("Any of the topics need to be chosen from those 6")
    print("Each of the topics has 6 questions")
...
topics = {1: "TV_shows_and_movies", 2: "Riddles", 3: "Mythology", 4: "Sports", 5: "Geography", 6: "General_Knowledge"}
print(topics)
chosen = int(input("\nSelect any topic from 1 to 6 in the given topics: "))

if chosen not in topics:
    print(Fore.RED + "Invalid topic selected. Please restart the game and select a valid topic.")
    exit()

selected_topic = topics[chosen]
...
def player_bid(player_name, player_money):
    ...
    while bid <= 0 or bid > player_money:
        print(Style.RESET_ALL)
        bid = int(input("Enter a valid amount to bid: "))
    return bid

questions = []
with open(r"c:\Users\Danduchandrashikara\OneDrive\Desktop\Bidding game\info.json", "r") as f:
    d = json.load(f)
...
def hint(selected_topic, i):
    hints = {
        "TV_shows_and_movies": {
            "Who lifted Mjolnir in avengers endgame?": "the role played by Chris Evans",
            ...
            "The song to win the oscar in the best original song category 2023?": "Ram charan and NTR are actors",
        ...
        "Riddles": {
            "What is always in front of you but can’t be seen?": "it is related to tenses in english grammar",
        ...
        "Mythology": {
            "Which one is the first avatar of lord Vishnu?": "another name of fish",
        ...
        "Sports": {
            "What is the beginning level in karate?": "usually wears it to waist",
        ...
        "Geography": {
            "Which country is the largest in the world in terms of surface area?": "the moscow is the capital of that country",
        ...
        "General_knowledge": {
            "Which is the sensitive organ in our body?": "the body's largest organ",
            ...
            "Who represents statue of equality": "he is a vedic philosopher and social reformer who revived bhakti movement"
        }
    }
...
def ques(a, player_money):
    ...
    for i in a:
        ...
        if answ == dict[i]:
            ...
            engine.runAndWait()
            engine.stop()
            player_money += player_bid_amount
        else:
            ...
            player_money -= player_bid_amount
            if player_money <= 0:
                engine.say("you are out of the game")
                print(Fore.LIGHTRED_EX + "You are out of the game!!!")
                exit()
            print(Style.RESET_ALL)
        print(f"{player_name} has {player_money} in your account")
        print(Style.RESET_ALL)
